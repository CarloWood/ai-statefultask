...
void AIStatefulTask::yield(void)
{
  ...
  mYieldEngine = state_r->current_engine ? state_r->current_engine : (mDefaultEngine ? mDefaultEngine : &gStatefulTaskThreadEngine);
}
...
AIEngine gStatefulTaskThreadEngine("gStatefulTaskThreadEngine");

// Stateful Task Thread main loop.
...
class AIEngineThread : public AIThread
{
  public:
...
AIEngineThread::AIEngineThread(void) : AIThread("AIEngineThread"), mRunning(true)
{
}
...
void AIEngineThread::run(void)
{
  while(mRunning)
  {
    gStatefulTaskThreadEngine.threadloop();
  }
}
...
void stopEngineThread(void)
{
  ...
  gStatefulTaskThreadEngine.wake_up();
  int count = 401;
  while(--count && !AIEngineThread::sInstance->isStopped())
  {
    ms_sleep(10);
  }
  Dout(dc::notice, "Stateful task thread" << (!AIEngineThread::sInstance->isStopped() ? " not" : "") << " stopped after " << ((400 - count) * 10) << "ms.");
}
#endif

#ifdef CWDEBUG
NAMESPACE_DEBUG_CHANNELS_START
channel_ct task("STATEFULTASK");
